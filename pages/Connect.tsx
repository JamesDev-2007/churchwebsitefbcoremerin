...
const Connect: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const livePosts: Post[] = snapshot.docs.map(doc => ({
        id: doc.id,
        name: doc.data().authorName,
        type: doc.data().type,
        message: doc.data().content,
        timestamp: doc.data().createdAt.toDate(),
        interactions: doc.data().likes || 0,
      }));
      setPosts(livePosts);
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        ...
        grouped[data.postId].push(`${data.userName}: ${data.message}`);
      });
      setComments(grouped);
    });
    return () => unsubscribe();
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        const data = doc.data();
        if (!grouped[data.postId]) grouped[data.postId] = [];
        grouped[data.postId].push(data.userName);
      });
      setLikedBy(grouped);
    });
    return () => unsubscribe();
  ...
  const handlePostSubmit = async (e: React.FormEvent) => {
    ...
    const newPost = {
      authorId: 'user123',
      authorName: isAnonymous ? 'Anonymous' : name.trim() || 'Anonymous',
      type: postType,
  ...
  const handleInteraction = async (postId: string) => {
    const userId = 'user123';
    const userName = name || 'Anonymous';

    const likeRef = collection(db, 'likes');
    const existing = await getDocs(query(likeRef, where('postId', '==', postId), where('userId', '==', userId)));

    if (existing.empty) {
      await addDoc(likeRef, { postId, userId, userName });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-warm-gray dark:bg-gray-700 p-6 rounded-xl shadow-md mb-10">
          <form onSubmit={handlePostSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="postType" value="Prayer" checked={postType === 'Prayer'} onChange={() => setPostType('Prayer')} className="h-4 w-4" />
                🙏 Prayer Request
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="postType" value="Testimony" checked={postType === 'Testimony'} onChange={() => setPostType('Testimony')} className="h-4 w-4" />
                🙌 Testimony
              </label>
            </div>
        ...
        <div className="space-y-6">
          {filteredPosts.map(post => (
            <div key={post.id} className="bg-warm-gray dark:bg-gray-700 p-5 rounded-lg shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${post.type === 'Prayer' ? 'bg-blue-200 text-blue-800' : 'bg-green-200 text-green-800'}`}>
              ...
              {/* Liked By */}
              {likedBy[post.id]?.length > 0 && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
                  Liked by: {likedBy[post.id].join(', ')}
                </p>
              )}
              ...
              <div className="mt-4 space-y-2">
                {comments[post.id]?.map((c, i) => (
                  <p key={i} className="text-sm text-gray-600 dark:text-gray-400">{c}</p>
                ))}
                <form onSubmit={(e) => { e.preventDefault(); handleCommentSubmit(post.id); }}>
                  <input
                    ...
                    className="w-full px-3 py-2 mt-2 border rounded-md bg-white dark:bg-gray-600 dark:border-gray-500"
                  />
                </form>
              </div>
            </div>
