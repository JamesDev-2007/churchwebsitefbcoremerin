...
const eventCategories: EventCategory[] = ['Regular Service', 'Departmental Meetings/Programs', 'Special Event'];

const getCategoryColor = (category: EventCategory): string => {
    switch (category) {
        case 'Regular Service':
            return 'bg-church-maroon border-church-maroon';
        case 'Departmental Meetings/Programs':
...
const createIcsFile = (event: Event) => {
    ...
    let startTime = new Date(`${event.date} ${startTimeStr}`);
    let endTime: Date;

    if (endTimeStr) {
        endTime = new Date(`${event.date} ${endTimeStr}`);
    } else {
...
// Helper to format time for calendar view
const formatStartTime = (time: string): string => {
    const startTime = time.split(' - ')[0]; // "7:20 AM"
    return startTime.replace(/\s/g, ''); // "7:20AM"
};
                        ...
                        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-lg printable-calendar">
                            ...
                            <div className="mb-4 flex flex-wrap justify-center gap-2 no-print">
                                {eventCategories.map(cat => (
                                    <button key={cat} onClick={() => handleFilterToggle(cat)} className={`px-3 py-1 text-xs font-semibold rounded-full transition-all ${activeFilters.includes(cat) ? `bg-church-maroon text-white` : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
                                        {cat}
                                    </button>
                                ))}
                                        ...
                                        <div key={day} className={`border rounded-lg p-1.5 min-h-[100px] sm:min-h-[120px] transition-colors ${isToday ? 'bg-yellow-100 dark:bg-yellow-900/30 border-yellow-400' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700/50'}`}>
                                            ...
                                            <div className="mt-1 space-y-1">
                                                {(eventsByDate[day] || []).map(event => (
                                                    <button key={event.id} onClick={() => setSelectedEvent(event)} className={`w-full text-left text-white text-[10px] px-1 py-0.5 rounded overflow-hidden flex items-center gap-1 ${getCategoryColor(event.category)}`}>
                                                        <span className="font-semibold flex-shrink-0">{formatStartTime(event.time)}</span>
                        ...
                        <div className="space-y-4">
                            {upcomingEvents.map(event => (
                                <div key={event.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md flex items-start space-x-4">
                                    <div className={`flex-shrink-0 w-12 h-12 text-center text-white rounded-md flex flex-col justify-center items-center bg-church-maroon`}>
                ...
                <Modal isOpen={!!selectedEvent} onClose={() => setSelectedEvent(null)}>
                    <div>
                        ...
                        <div className="p-6 md:p-8">
                            <span className={`px-3 py-1 text-xs font-semibold rounded-full text-white ${getCategoryColor(selectedEvent.category)}`}>{selectedEvent.category}</span>
