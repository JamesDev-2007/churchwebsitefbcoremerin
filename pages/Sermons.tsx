...
const Sermons: React.FC = () => {
    ...
    const resetFilters = () => {
        setSearchTerm('');
        setPreacherFilter('All Preachers');
        setTagFilter('All Topics');
        setStartDate('');
        setEndDate('');
    };
        ...
        const handler = setTimeout(async () => {
            const applyLocalFilters = (sermonsToFilter: Sermon[]) => {
                ...
                if (startDate) {
                    ...
                    filtered = filtered.filter(s => new Date(`${s.date}T00:00:00`) >= start);
                }
                if (endDate) {
                    const end = new Date(`${endDate}T00:00:00`);
                    filtered = filtered.filter(s => new Date(`${s.date}T00:00:00`) <= end);
                }
                return filtered;
            };
    ...
    }, [searchTerm, preacherFilter, tagFilter, startDate, endDate]);
        ...
        <div className="mb-8 max-w-4xl mx-auto space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                <div>
                    <label htmlFor="start-date" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 ml-2">From</label>
                    <input
                        type="date"
                        id="start-date"
                        value={startDate}
                ...
                <div>
                    <label htmlFor="end-date" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 ml-2">To</label>
                    <input
                        type="date"
                        id="end-date"
                        value={endDate}
             ...
             <div className="text-right pt-2">
                <button onClick={resetFilters} className="text-sm font-semibold text-church-maroon-dark dark:text-yellow-300 hover:underline">
                    Reset Filters
                </button>
            </div>
        </div>
